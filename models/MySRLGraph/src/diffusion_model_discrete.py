...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        if cfg.model.transition == 'uniform':
            ...
            x_limit = torch.ones(self.Xdim_output) / max(1, self.Xdim_output)
            e_limit = torch.ones(self.Edim_output) / max(1, self.Edim_output)
            y_limit = torch.ones(self.ydim_output) / max(1, self.ydim_output)
            self.limit_dist = utils.PlaceHolder(X=x_limit, E=e_limit, y=y_limit)
        elif cfg.model.transition == 'marginal':

            if getattr(self.cfg.model, 'edge_only', False):
                x_marginals = torch.ones(1)
            else:
    ...
    def validation_step(self, data, i):
        ...
        dense_data = dense_data.mask(node_mask)
        if getattr(self.cfg.model, 'edge_only', False):
            noisy_data = self.apply_noise_edge_only(dense_data.X, dense_data.E, data.y, node_mask)
        else:
    ...
    def kl_prior_edge_only(self, X, E, node_mask):
        ...
        bs, n, n, _ = probE.shape
        limit_E = self.limit_dist.E[None, None, None, :].expand(bs, n, n, -1).type_as(probE)

        # Make sure that masked rows do not contribute to the loss
        limit_dist_E, probE = diffusion_utils.mask_distributions_edge_only(true_E=limit_E.clone(),
                                                                           pred_E=probE,
                                                                           node_mask=node_mask)
    ...
    def compute_Lt_edge_only(self, X, E, y, pred, noisy_data, node_mask, test):
        pred_probs_E = F.softmax(pred.E, dim=-1)
        ...
        bs, n, d = X.shape
        prob_true_E = diffusion_utils.compute_posterior_distribution(M=E, M_t=noisy_data['E_t'], Qt_M=Qt.E, Qsb_M=Qsb.E, Qtb_M=Qtb.E)
        prob_true_E = prob_true_E.reshape((bs, n, n, -1))
        
        prob_pred_E = diffusion_utils.compute_posterior_distribution(M=pred_probs_E, M_t=noisy_data['E_t'], Qt_M=Qt.E, Qsb_M=Qsb.E, Qtb_M=Qtb.E)
        prob_pred_E = prob_pred_E.reshape((bs, n, n, -1))

        prob_true_E, prob_pred_E = diffusion_utils.mask_distributions_edge_only(true_E=prob_true_E,
                                                                                pred_E=prob_pred_E,
                                                                                node_mask=node_mask)
    ...
    def reconstruction_logp_edge_only(self, t, X, E, node_mask):
        t_zeros = torch.zeros_like(t)
        beta_0 = self.noise_schedule(t_zeros)
        Q0 = self.transition_model.get_Qt(beta_t=beta_0, device=self.device)
        ...
        sampled0_E = diffusion_utils.sample_edge_only_features(probE=probE0, node_mask=node_mask)

        E0 = F.one_hot(sampled0_E, num_classes=self.Edim_output).float()
        X0 = X  # Keep X frozen
        
        sampled_0 = utils.PlaceHolder(X=X0, E=E0, y=torch.zeros(X0.shape[0], 0).type_as(E0)).mask(node_mask)

        # Predictions
        noisy_data = {'X_t': sampled_0.X, 'E_t': sampled_0.E, 'y_t': sampled_0.y, 'node_mask': node_mask,
                      't': torch.zeros(X0.shape[0], 1).type_as(E0)}
        extra_data = self.compute_extra_data(noisy_data)
        pred0 = self.forward(noisy_data, extra_data, node_mask)
        ...
        return utils.PlaceHolder(X=X0, E=probE0, y=torch.zeros(X0.shape[0], 0).type_as(probE0))
    ...
    def apply_noise_edge_only(self, X, E, y, node_mask):
        """ Sample noise and apply it to the data. Only diffuses edges. """
        lowest_t = 0 if self.training else 1
        t_int = torch.randint(lowest_t, self.T + 1, size=(X.size(0), 1), device=X.device).float()  # (bs, 1)
        s_int = t_int - 1
        ...
        sampled_E = diffusion_utils.sample_edge_only_features(probE=probE, node_mask=node_mask)

        E_t = F.one_hot(sampled_E, num_classes=self.Edim_output).float()
        X_t = X  # Keep X frozen
        assert (E.shape == E_t.shape)

        z_t = utils.PlaceHolder(X=X_t, E=E_t, y=y).type_as(E_t).mask(node_mask)
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        if getattr(self.cfg.model, 'edge_only', False):
            ...
            if batch is not None:
                ...
                n_max = dense_data.X.size(1)
                bs_actual = min(batch_size, dense_data.X.size(0))
                
                X = dense_data.X[:bs_actual].to(self.device)
                node_mask = node_mask[:bs_actual].to(self.device)
                y = batch.y[:bs_actual].to(self.device) if batch.y is not None else torch.zeros(bs_actual, 0).to(self.device)
                
                n_nodes = node_mask.sum(1)
                batch_size = bs_actual
                
                E_T = diffusion_utils.sample_edge_only_noise(limit_dist_E=self.limit_dist.E, node_mask=node_mask)
                E = E_T
            else:
                return []
        else:
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            # Sample z_s
            if getattr(self.cfg.model, 'edge_only', False):
                sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt_edge_only(s_norm, t_norm, X, E, y, node_mask)
            else:
    ...
    def sample_p_zs_given_zt_edge_only(self, s, t, X_t, E_t, y_t, node_mask):
        """Samples from zs ~ p(zs | zt) for edges only."""
        bs, n, dxs = X_t.shape
        ...
        E_s = F.one_hot(sampled_E_s, num_classes=self.Edim_output).float()

        out_one_hot = utils.PlaceHolder(X=X_t, E=E_s, y=torch.zeros(y_t.shape[0], 0).type_as(y_t))
        out_discrete = utils.PlaceHolder(X=X_t, E=E_s, y=torch.zeros(y_t.shape[0], 0).type_as(y_t))
