...
def sample_edge_only_features(probE, node_mask):
    """Sample edge features only (no node sampling), without symmetry enforcement.
    Used for SRL edge-only diffusion with directed edges.

    Args:
        probE: (bs, n, n, de_out) edge transition probabilities
        node_mask: (bs, n) boolean mask

    Returns:
        E_t: (bs, n, n) sampled edge class indices
    """
    ...
    # Sample E — NO symmetry enforcement (directed edges)
    E_t = probE.multinomial(1).reshape(bs, n, n)   # (bs, n, n)

    # Zero out diagonal (no self-loops)
    diag = torch.eye(n, dtype=torch.bool, device=E_t.device).unsqueeze(0).expand(bs, -1, -1)
    E_t[diag] = 0
...
def sample_edge_only_noise(limit_dist_E, node_mask):
    """Sample edge noise only from limit distribution, without symmetry enforcement.
    Used for SRL edge-only diffusion with directed edges.

    Args:
        limit_dist_E: (de,) edge class marginal distribution
        node_mask: (bs, n) boolean mask

    Returns:
        PlaceHolder with E_T as one-hot edge noise (bs, n, n, de)
    """
    bs, n_max = node_mask.shape
    e_limit = limit_dist_E[None, None, None, :].expand(bs, n_max, n_max, -1)

    U_E = e_limit.flatten(end_dim=-2).multinomial(1).reshape(bs, n_max, n_max)

    # Zero out diagonal (no self-loops)
    diag = torch.eye(n_max, dtype=torch.bool, device=node_mask.device).unsqueeze(0).expand(bs, -1, -1)
    U_E[diag] = 0

    U_E = U_E.type_as(node_mask.long())
    U_E = F.one_hot(U_E, num_classes=e_limit.shape[-1]).float()

    # No symmetry enforcement — directed edges

    # Mask invalid nodes
    e_mask1 = node_mask.unsqueeze(2).unsqueeze(3)   # (bs, n, 1, 1)
    e_mask2 = node_mask.unsqueeze(1).unsqueeze(3)   # (bs, 1, n, 1)
    U_E = U_E * e_mask1 * e_mask2

    return U_E


def mask_distributions_edge_only(true_E, pred_E, node_mask):
    """Mask edge distributions only (skip X masking for edge-only diffusion).

    Args:
        true_E: (bs, n, n, de_out)
        pred_E: (bs, n, n, de_out)
        node_mask: (bs, n)
